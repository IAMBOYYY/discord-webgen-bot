...
# -------------------------------------------------------------------
# !restore, !models, !setkey, !provider, !imgsetup, !imgmodels
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# !serverdetail, !banwords, !serverbackup, !serverrestore
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Auto-moderation + roast + mention handling in on_message
# -------------------------------------------------------------------
@bot.event
async def on_message(message):
    ...
    if message.guild:
        ...
        if key in roast_targets:
            count, roaster_id = roast_targets[key]
            if count > 0:
...
async def handle_roast(message, key):
    ...
    if client:
        ...
        try:
            ...
            roast_text = f"Oops, I couldn't roast you: {e}"

    if not roast_text or roast_text.isspace():
        roast_text = random.choice(FALLBACK_ROASTS)

    await message.reply(f"{target_user.mention} {roast_text}")
...
async def meme_command(ctx, *, texts: str):
    ...
    try:
        ...
        return await ctx.send(f"❌ Invalid image: {e}")

    draw = ImageDraw.Draw(image)
    # Try to load a nice font, fallback to default with size 40
    font = None
    try:
    ...
    def draw_text(draw, text, y_start, max_width):
        ...
        for word in words:
            ...
            if hasattr(draw, "textbbox"):
                ...
                w, _ = draw.textsize(test_line, font=font)  # old Pillow
            if w <= max_width:
                line = test_line
            else:
                lines.append(line)
...
# -------------------------------------------------------------------
# !ask – WITH WEB SEARCH + concise answers
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# !make (with Unsplash, simple structure)
# -------------------------------------------------------------------
            ...
            if msg.tool_calls:
                for tc in msg.tool_calls:
                    ...
                    if func == "create_file":
                        ...
                        filename = Path(filename).name   # strip directory
