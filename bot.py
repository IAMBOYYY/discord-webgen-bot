...
async def on_ready():
    ...
    # internal keep-alive pings itself every 5 minutes
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(url, headers=headers, timeout=10) as resp:
                ...
                # return list of dicts with id and capabilities
...
async def setup(ctx):
    ...
    try:
        ...
        await ctx.send("📁 Here is your backup file. If the bot restarts, use `!restore` with this file to get everything back.", file=file)
...
# -------------------------------------------------------------------
# !models, !setkey, !provider, !imgsetup, !imgmodels (complete but concise)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# !draw (same as before, no local file storage)
# -------------------------------------------------------------------
    ...
    if client:
        try:
            ...
            resp = await client.chat.completions.create(
                model=chat_model,
                messages=[{"role": "system", "content": "Turn the user's prompt into a detailed image generation prompt. Output only the prompt."}, {"role": "user", "content": prompt}],
        ...
        await ctx.send("⚠️ No chat client, using original prompt.")
...
async def attach_image(ctx, *, description: str = ""):
    ...
    for attempt in range(3):
        async with ctx.typing():
            try:
                resp = await client.chat.completions.create(model=model, messages=messages, temperature=0.7, max_tokens=256)
...
async def make_website(ctx, *, description: str):
    ...
    try:
        plan_resp = await client.chat.completions.create(
            model=model,
            messages=[{"role": "system", "content": "You are a planning AI. Outline steps and files needed for this project. Be concise."}, {"role": "user", "content": description}],
    ...
    for _ in range(3):
        resp = await client.chat.completions.create(model=model, messages=review_messages, tools=tools, tool_choice="auto", temperature=0.3)
        msg = resp.choices[0].message
...
async def improve_project(ctx):
    ...
    # similar to review phase in make
    tools = [
        {"type": "function", "function": {"name": "create_file", "parameters": {"type": "object", "properties": {"filename": {"type": "string"}, "content": {"type": "string"}}, "required": ["filename", "content"]}, "description": "Create a file"}},
        {"type": "function", "function": {"name": "read_file", "parameters": {"type": "object", "properties": {"filename": {"type": "string"}}, "required": ["filename"]}, "description": "Read a file"}},
        {"type": "function", "function": {"name": "list_files", "parameters": {"type": "object", "properties": {}}, "description": "List files"}},
