...
# Image providers
IMAGE_PROVIDERS = {
    "openrouter": {"name": "OpenRouter images", "base_url": "https://openrouter.ai/api/v1", "default_model": "stabilityai/stable-diffusion-xl", "api_type": "openai_images"},
    ...
    async with ctx.typing():
        try:
            if provider == "openrouter":
                ...
                url = f"{cfg['base_url']}/{prompt}?model={model}&key={api_key}"
                await ctx.send(f"🖼️ **{prompt}**\n{url}")
