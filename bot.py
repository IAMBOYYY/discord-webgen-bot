...
# -------------------------------------------------------------------
# !imgsetup, !imgmodels, !draw (already fixed above, included)
# -------------------------------------------------------------------
# (copied from earlier)

# -------------------------------------------------------------------
# !models, !setkey, !provider (unchanged)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# !make (multi-language, Unsplash, multi-page)
# -------------------------------------------------------------------
...
async def attach_image(ctx, *, description: str = ""):
    ...
    if attachment.size > 1_000_000:
        await ctx.send("⚠️ Image is over 1MB, which may cause errors. Consider compressing it.")
    ...
    async with ctx.typing():
        try:
            resp = await client.chat.completions.create(model=model, messages=messages, temperature=0.7, max_tokens=4096)
