...
bot.remove_command('help')   # Removes the default help command so we can use ours
...
# -------------------------------------------------------------------
# !imgsetup, !imgmodels, !draw, !attach, etc. (all remaining commands)
# (I'll include them completely to avoid missing any)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Admin cleanup (removes all project files and stray temp files)
# -------------------------------------------------------------------
    ...
    try:
        ...
        if msg.content == ADMIN_PASSWORD:
            for folder in list(workspace_meta.keys()):
                await delete_project(folder)
            # Delete any stray files in workspace root (e.g., generated images if any)
